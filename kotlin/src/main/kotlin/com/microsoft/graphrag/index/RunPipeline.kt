...
private suspend fun loadExistingState(storage: PipelineStorage): Map<String, Any?> {
    ...
    return Json.decodeFromString(stateSerializer, stateJson).toMutableMap()
}
    ...
    try {
        val stateJson =
            Json { prettyPrint = true }.encodeToString(
                stateSerializer,
                context.state.mapValues { it.value?.toString() },
            )
 ...
 */
private val stateSerializer = MapSerializer(String.serializer(), String.serializer().nullable)
